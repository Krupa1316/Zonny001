...
def _enforce_path_safety(path: str, project_root: str) -> tuple:
    ...
    try:
        ...
        return None, f"🚫 Path blocked: '{path}' is outside the workspace root"
...
def _check_write_permission(path: str) -> tuple:
    ...
    if PERMISSION_MODE == "safe":
        return False, "🔒 Write blocked: Zonny is running in safe mode (ZONNY_MODE=safe)"

    ext = os.path.splitext(path)[1].lower()

    if ext in WRITE_BLOCKED_EXTENSIONS:
        return False, f"🚫 Write blocked: '{ext}' files are never allowed"

    # Dotfiles with no extension: block known sensitive names regardless
    basename = os.path.basename(path).lower()
    if basename in (".env", ".secret", ".credentials", ".htpasswd"):
        return False, f"🚫 Write blocked: '{basename}' is a sensitive dotfile"

    if PERMISSION_MODE == "dev" and ext and ext not in WRITE_ALLOWED_EXTENSIONS:
        return False, f"🚫 Write blocked: '{ext}' is not in the allowed extension list"
...
def dispatch(intent: dict, context: dict) -> str:
    ...
    if tool_name and not get_tool_by_name(tool_name):
        return f"❌ Unknown tool: {tool_name} (not in tool registry)"
    ...
    if tool_name == "filesystem.write":
        ...
        try:
            confirm = input(f"\n⚠️  Zonny wants to write to '{path}'. Approve? [y/N] ").strip().lower()
...
{git_info.get('status', 'Clean')}"""
    ...
    if tool_name == "code.analyze":
        ...
        try:
            ...
            return f"📄 Contents of {file}:\n\n{content[:1000]}...\n\n💡 Tip: Full code analysis requires the agent system."
        except Exception as e:
            return f"❌ Error reading file: {e}"
    ...
    if tool_name == "memory.recall":
        ...
        if agent:
            return agent.run(query, context)
        return "🧠 Memory system requires the agent system with memory.py module."
