...
def handle_system_command(command: str, session_id: str, context: Dict) -> Optional[Dict]:
    ...
    if cmd_lower == '/help':
        return {
            "response": """🤖 Zonny System Commands
...
Active Components:
  • Router: ✅ Ready
  • Memory: ✅ Connected
  • Ollama: ✅ nemotron-3-nano:latest (120s timeout)
"""
    ...
    # /git - Git status
        ...
        if not git_info.get('is_git_repo'):
            return {"response": "❌ Not a git repository"}
...
"""
    ...
    # /clear - Clear conversation (keep session)
        ...
        return {
            "response": "✨ Conversation cleared (session preserved)",
    ...
    # /reset - Full reset
        ...
        return {
            "response": "🔄 System reset complete",
            "action": "reset_system"
        }
